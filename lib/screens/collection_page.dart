...
class _CollectionPageState extends State<CollectionPage> {
  ...
  Future<void> _startRecording() async {
    print("--- 1. Start Recording button pressed. ---");
    ...
    print("--- 2. Memulai proses perekaman (menggunakan direktori privat aplikasi). ---");
    ...
    print("--- 3. Sensor listeners telah dimulai. ---");
  }
  ...
  Future<void> _stopRecording() async {
    ...
    try {
      ...
      if (directory == null) throw "Cannot get storage directory";
      ...
      print("SUCCESS: Data saved to ${file.path}. Initiating share dialog.");
  ...
  // --- FUNGSI BARU: Memicu Dialog Sharing Sistem ---
  Future<void> _shareFile(String filePath, String fileName) async {
    try {
      final file = XFile(filePath); // Buat objek XFile dari path lokal

      // Memicu dialog share sistem. Pengguna dapat memilih Google Drive, Email, dll.
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Dialog Share dibuka. Pilih Google Drive untuk menyimpan!"))
        );
      }
    } catch (e) {
      print("Error sharing file: $e");
      if (mounted) {
        // Jika share gagal (misalnya tidak ada aplikasi share), tampilkan error.
        ScaffoldMessenger.of(context).showSnackBar(
